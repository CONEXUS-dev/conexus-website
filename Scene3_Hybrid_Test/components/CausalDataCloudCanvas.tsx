...
const BASE_SCALE_MIN = 0.14;
const BASE_SCALE_RANGE = 0.13;
...
varying float vPhysicalActivation;
varying float vSeed;
...
void main() {
  ...
  // High-frequency temporal quiver is presentation-only at rest. Focus-driven
  // amplification is scoped to the matching condition.
  ...
  float physicalActivation = uEnergy * uFocusAmount * focused;
  float escapeEnergy = physicalActivation * aEscape * (0.35 + 0.65 * aExcess);
  ...
  vExcess = aExcess;
  vEscape = aEscape;
  vPhysicalActivation = physicalActivation;
  vSeed = aSeed;
}
`;
...
void main() {
  ...
  // Equation-derived escape excess under condition-scoped physical activation.
  float escapeEnergy = vPhysicalActivation * vEscape * (0.4 + 0.6 * vExcess);
...
class VaultAudioEngine {
  ...
  private buildGraph(): void {
    ...
    saturate.curve = SATURATION_CURVE as Float32Array<ArrayBuffer>;
    ...
    shaperCrush.curve = BITCRUSH_CURVE as Float32Array<ArrayBuffer>;
...
interface SceneRuntimeState {
  /** Smoothed focus energy in [0, 1]; physical use is scoped per run. */
  energy: number;
  ...
  focusAmount: number;
  /** User preference; disables autonomous motion without changing evidence. */
...
/* --- Focus / camera tuning (classification constants remain locked above) --- */
const ELEMENT_RETRY_MS = 600;
const ENERGY_SMOOTH_RATE = 3.2;
...
/* --- Cinematic camera --- */
const CAMERA_RADIUS_REST = 25;
const CAMERA_RADIUS_PULL = 7;
const CAMERA_PHI_REST = 1.44;
const CAMERA_PHI_DROP = 0.08;
const CAMERA_DRIFT_RATE = 0.006;
const CAMERA_PARALLAX_AZIMUTH = 0.09;
const CAMERA_PARALLAX_PHI = 0.055;
const CAMERA_POINTER_RATE = 2.5;
const CAMERA_EVOLVE_RATE = 1.4;

/* --- Postprocessing targets --- */
const BLOOM_BASE = 0.55;
const BLOOM_ENERGY_GAIN = 0.6;
const BLOOM_FOCUS_GAIN: readonly number[] = [0.15, 0.12, 0.45, 2.15];
...
/**
 * Tracks scene visibility, pointer parallax, and reduced-motion preference.
 * Focus—not document scroll—drives causal activation.
 */
...
function PhysicsField() {
  ...
  useFrame((state, delta) => {
    ...
    // Causal energy is zero at rest and rises only for an explicit condition.
    const activeArm = useDataVaultStore.getState().activeArm;
    const activeArmIndex = ARM_INDEX[activeArm];
    if (activeArmIndex === -1) {
      RUNTIME.energy = 0;
    } else {
      RUNTIME.energy +=
        (1 - RUNTIME.energy) * (1 - Math.exp(-dt * ENERGY_SMOOTH_RATE));
    }
    ...
    const presentationEnergy = RUNTIME.energy;
    ...
    for (let i = 0; i < SCENE3_RUN_COUNT; i++) {
      ...
      // Physical activation is condition-scoped. Ghosted populations never
      // execute another condition's causal demonstration.
      const armMatch =
        activeArmIndex >= 0 && focus.arm === activeArmIndex && arm[i] === activeArmIndex
          ? 1
          : 0;
      const physicalActivation = presentationEnergy * focusEase * armMatch;
      const homeBlend = 1 - physicalActivation;

      // Softened gravity and every regime force use per-run activation.
      const gMag = (GM / (r * r + GRAVITY_SOFTEN)) * physicalActivation;
      ax -= dx * gMag;
      ay -= dy * gMag;
      az -= dz * gMag;
      ...
      if (escaped[i] === 1) {
        ...
        const thrust = ESCAPE_THRUST * (0.4 + 0.6 * excess[i]) * physicalActivation;
        ax += dx * thrust;
        ay += dy * thrust;
        az += dz * thrust;

        const swirl = ESCAPED_SWIRL * physicalActivation;
        ax += tangent[i3] * swirl;
        ...
        const radial = (rTarget - r) * SPRING_TRAPPED * physicalActivation;
        ax += dx * radial;
        ay += dy * radial;
        az += dz * radial;

        const swirl = TRAPPED_SWIRL * physicalActivation;
        ax += tangent[i3] * swirl;
        ay += tangent[i3 + 1] * swirl;
        az += tangent[i3 + 2] * swirl;
        drag = DRAG_TRAPPED;
      ...
      let damp = Math.exp(-dt * drag * (0.2 + 0.8 * physicalActivation));
      if (homeBlend > 0.001) {
      ...
      const depthScale = clamp(0.82 + (12 - radius0[i]) * 0.04, 0.72, 1.28);
      const s = baseScale[i] * depthScale;
      const wanderAmp = RUNTIME.reducedMotion ? 0 : homeBlend * REST_WANDER_AMP;
    ...
    material.uniforms.uEnergy.value = presentationEnergy;
...
function CentroidCore() {
  ...
  useFrame((state) => {
    ...
    const mesh = meshRef.current;
    if (mesh) {
      const pulse = 1 + 0.05 * Math.sin(time * 1.7 + Math.sin(time * 0.53) * 1.3) + 0.22 * energy;
...
function CameraRig() {
  ...
  useFrame((state, delta) => {
    ...
    const azimuth =
      ...
      (RUNTIME.reducedMotion ? 0 : Math.sin(time * 0.1) * 0.025);
    const phi = clamp(
      rig.phi - rig.pointerY * CAMERA_PARALLAX_PHI +
        (RUNTIME.reducedMotion ? 0 : Math.sin(time * 0.13) * 0.012),
      0.32,
      1.46,
    );
    ...
    const handheldX = RUNTIME.reducedMotion
      ? 0
      : Math.sin(time * 0.27) * 0.045 + Math.sin(time * 0.61) * 0.015;
    const handheldY = RUNTIME.reducedMotion ? 0 : Math.cos(time * 0.23) * 0.035;
